...
# ----------------------------------------------------------------------------------
# gxpo Segment Tree Classes and Helpers (faithful copy from sync_ds_multi)
# ----------------------------------------------------------------------------------
...
class Segment:
    """A sequence of actions forming a node in the gxpo search tree."""
    def __init__(self, actions: list, parent: Optional['Segment'] = None):
        ...
        # gxpo metrics calculated at the start of each generation loop
        self.advantage = 0.0
        self.branch_se = 0.0
    @property
...
# ----------------------------------------------------------------------------------
# Explore-only splitting helpers (ported from gxpo_vllm_async_simplified)
# ----------------------------------------------------------------------------------
...
def generate_rollouts_from_segment(segment: Segment, num_rollouts: int, item, model_gen, model_ref, tokenizer, reward_fn, max_gen_tokens, prompt_len_root, device):
    ...
    per-token statistics needed for gxpo.
...
def get_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Use GSPO-style sequence likelihood on the active tail segment (not the prefix)"
    )
...
debug = args.debug
use_sequence_likelihood = args.use_sequence_likelihood
...
# Enforce single prompt per train batch so advantages are normalized per-prompt
assert num_prompts == 1, "gxpo requires num_prompts == 1 for correct advantage normalization"
...
def main():
    ...
    for step in trange(1, args.steps + 1, desc="Training Steps", disable=(extended_rank != 0)):
        ...
        for mb_idx in range(num_micro_batches):
            ...
            if use_sequence_likelihood:
                # GSPO-style: sequence likelihood over the active tail, length-normalized
                curr_seq_list, gen_seq_list, ref_seq_list, seq_len_list = [], [], [], []
                for i in range(mb_full_logps.shape[0]):
                    p_len = mb_prompt_lens[i]
                    ans_len = len(mb_ans_token_ids[i])
                    tail = mb_full_logps[i, p_len - 1 : p_len - 1 + ans_len].float()
                    gen_tail = mb_gen_old[i, :ans_len].float()
                    ref_tail = mb_ref_logps[i, :ans_len].float()
                    curr_seq_list.append(tail.sum())
                    gen_seq_list.append(gen_tail.sum())
                    ref_seq_list.append(ref_tail.sum())
                    seq_len_list.append(float(ans_len))

                curr_seq = torch.stack(curr_seq_list)
                gen_seq = torch.stack(gen_seq_list)
                ref_seq = torch.stack(ref_seq_list)
                seq_lens = torch.tensor(seq_len_list, device=curr_seq.device, dtype=torch.float32).clamp(min=1.0)

                log_r = torch.clamp((curr_seq - gen_seq.detach()) / seq_lens, min=-20.0, max=20.0)
                log_r_ref = torch.clamp((ref_seq.detach() - curr_seq) / seq_lens, min=-20.0, max=20.0)

                seq_ratios = torch.exp(log_r)
                seq_clipped = torch.clamp(seq_ratios, 1 - clip_param, 1 + clip_param)
                # mb_advantages is [mb]
                seq_pg_loss = -torch.min(seq_ratios * mb_advantages, seq_clipped * mb_advantages)  # [mb]
                seq_kl = torch.exp(log_r_ref) - log_r_ref - 1  # [mb]

                mb_loss = (seq_pg_loss + beta * seq_kl).mean()
                mb_loss = mb_loss / gradient_accumulation_steps
                mb_loss.backward()

                microbatch_losses.append(mb_loss.detach().cpu())
                microbatch_pg_losses.append(seq_pg_loss.detach().cpu().flatten())
                microbatch_kl.append(seq_kl.detach().cpu().flatten())
                microbatch_ratios.append(seq_ratios.detach().cpu().flatten())
            else:
                # PPO loss computation with numerical stability (per-token)
        ...
        if extended_rank == 0:
            ...
            if use_sequence_likelihood:
                writer.add_scalar("train/seq_ratios/mean", ratios_all.mean().item(), step)
                writer.add_scalar("train/seq_ratios/std", ratios_all.std().item(), step)
                writer.add_scalar("train/seq_kl/mean", kl_all.mean().item(), step)
                writer.add_scalar("train/seq_kl/std", kl_all.std().item(), step)
                writer.add_scalar("train/seq_pg_loss/mean", pg_loss_all.mean().item(), step)
                writer.add_scalar("train/seq_pg_loss/std", pg_loss_all.std().item(), step)
            else:
