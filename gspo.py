...
def main():
    ...
    for step in trange(1, args.steps + 1, desc="Training Steps", disable=(extended_rank != 0)):
        ...
        curr_seq_logps = torch.stack(curr_seq_logps)
        gen_seq_logps = torch.stack(gen_seq_logps)
        ref_seq_logps = torch.stack(ref_seq_logps)
        seq_lengths = torch.tensor(seq_lengths, device=device, dtype=torch.float32)

        # GSPO: Sequence-level importance ratios and clipping
        seq_ratios = torch.exp(curr_seq_logps - gen_seq_logps.detach())
        ...
        # GSPO: Sequence-level KL regularization
        seq_kl = torch.exp(ref_seq_logps - curr_seq_logps) - (ref_seq_logps - curr_seq_logps) - 1
        
        # Normalize by sequence length to handle length bias
        normalized_pg_loss = seq_pg_loss / seq_lengths
        normalized_kl = seq_kl / seq_lengths
        
        # Final sequence-level loss
        seq_loss = normalized_pg_loss + beta * normalized_kl
        loss = seq_loss.mean()
