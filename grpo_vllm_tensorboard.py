    ...
    for it in range(999999999):
        ...
        if writer and it % 20 == 0:
            try:
                ...
                table_str = "| " + " | ".join(gen_table_columns) + " |\n"
                table_str += "| " + " | ".join(["---"] * len(gen_table_columns)) + " |\n"
                for i in range(len(inputs)):
                    ...
                    for j in range(num_pre_Q):
                        ...
                        rc_val = rewards_correct_grouped[i, j].item()
                        rf_val = rewards_format_grouped[i, j].item()
                        table_str += f"| {p_text} | {a_text} | {r_val:.4f} | {adv_val:.4f} | {rc_val:.4f} | {rf_val:.4f} |\n"
